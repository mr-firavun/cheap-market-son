...
import {
  ...
  AlertCircle, User, Lock, Phone, Eye, EyeOff, Shield, ShieldCheck, Bell, X,
} from 'lucide-react';
...
export default function DashboardPage({ onNavigate }: Props) {
  ...
  async function loadDashboardData() {
    if (!user) return;
    const [invRes, txRes, addrRes, notifRes] = await Promise.all([
  ...
  useEffect(() => {
    if (!user) return;
    // Trigger edge function to auto-process completed investments
    const supabaseUrl = import.meta.env.VITE_SUPABASE_URL as string;
    const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY as string;
    fetch(`${supabaseUrl}/functions/v1/process-completed-investments`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${supabaseAnonKey}`, 'Content-Type': 'application/json' },
    }).then(() => {
      // Refresh data after processing
      loadDashboardData().then(() => { refreshProfile(); setLoading(false); });
    }).catch(() => {
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div className="relative">
              <button
                ...
                className="relative w-10 h-10 flex items-center justify-center bg-gray-900 hover:bg-gray-800 border border-gray-700 rounded-xl transition-all"
              >
              ...
              {showNotifications && (
                <div className="absolute right-0 top-12 w-80 bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl z-50 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800">
                    ...
                    <button onClick={() => setShowNotifications(false)} className="text-gray-500 hover:text-gray-300 transition-colors">
                      <X size={15} />
                    </button>
                  </div>
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.length === 0 ? (
                      <div className="px-4 py-8 text-center text-gray-600 text-sm">Bildirim yok</div>
                    ) : (
                      notifications.map((n) => (
                        <div key={n.id} className={`px-4 py-3 border-b border-gray-800/60 last:border-0 ${!n.is_read ? 'bg-amber-500/5' : ''}`}>
                          <div className="flex items-start gap-2">
                            <div className={`mt-0.5 w-2 h-2 rounded-full shrink-0 ${n.type === 'profit' ? 'bg-emerald-400' : n.type === 'referral_bonus' ? 'bg-sky-400' : 'bg-amber-400'}`} />
                            <div className="min-w-0">
                              ...
                              <div className="text-xs text-gray-600 mt-1">{new Date(n.created_at).toLocaleDateString('tr-TR')}</div>
                            </div>
                          </div>
                        </div>
                      ))
            ...
            {profile?.is_admin && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-400 text-sm font-semibold rounded-xl transition-all"
              >
                <ShieldCheck size={16} />
                Admin Paneli
              </button>
          ...
          <div className="space-y-6">
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <div className="bg-gray-900/70 border border-gray-800 rounded-xl p-5">
                ...
                <div className="flex items-start gap-2.5 bg-sky-500/10 border border-sky-500/20 rounded-lg p-3 mb-3">
                  ...
                  <p className="text-xs text-sky-300 leading-relaxed">
                    When someone signs up with your link and makes a <strong className="text-sky-200">$200+ deposit</strong>, you instantly receive <strong className="text-sky-200">$15 USDT</strong> in your balance.
                  </p>
                </div>
                <div className="bg-gray-800 rounded-lg p-3 mb-3">
                  <div className="text-xs text-gray-400 mb-0.5">Your Referral Code:</div>
                  <div className="text-sm font-mono font-bold text-amber-400">{profile?.referral_code}</div>
                </div>
