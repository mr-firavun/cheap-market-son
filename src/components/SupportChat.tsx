...
export default function SupportChat() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      {/* Chat panel */}
      {open && (
        <div className="w-80 sm:w-96 bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between bg-gray-800 border-b border-gray-700 px-4 py-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="text-sm font-semibold text-white">CheapMarket Support</div>
      ...
      <button
        ...
        className={`w-14 h-14 rounded-2xl shadow-2xl flex items-center justify-center transition-all ${
          open ? 'bg-gray-700 hover:bg-gray-600 shadow-gray-900/50' : 'bg-amber-500 hover:bg-amber-400 shadow-amber-500/30'
        }`}
      >
        {open
          ? <X size={22} className="text-white" />
          : <MessageCircle size={22} className="text-gray-950" />
        }
      </button>
    </div>
  );
